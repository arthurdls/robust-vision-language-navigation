...
# ---------------------------------------------------------------------------
# Task loading (same as run_integration.py)
# ---------------------------------------------------------------------------
...
def _resolve_tasks(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    if task_file is not None:
        ...
        if not path.is_absolute():
            path = CONDITION4_TASKS_DIR / path.name
        if not path.exists():
    ...
    CONDITION4_TASKS_DIR.mkdir(parents=True, exist_ok=True)
    json_files = sorted(glob.glob(str(CONDITION4_TASKS_DIR / "*.json")))
    if not json_files:
        raise SystemExit(f"No JSON files found in {CONDITION4_TASKS_DIR}")
    tasks = []
