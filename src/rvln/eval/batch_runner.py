...
import cv2
import gym
import gym_unrealcv  # noqa: F401 -- triggers gym env registration
...
def draw_2d_trajectory_from_log(
    log_path: str,
    save_path: str,
    instruction: str,
    target: Optional[List[float]],
    tick_x: Optional[float] = None,
    tick_y: Optional[float] = None,
    min_span_x: Optional[float] = None,
    min_span_y: Optional[float] = None,
) -> None:
...
def draw_3d_trajectory_from_log(
    ...
    min_span_x: Optional[float] = None,
    min_span_y: Optional[float] = None,
    min_span_z: Optional[float] = None,
) -> None:
                ...
                for i, action_pose in enumerate(action_poses):
                    ...
                    if last_pose is not None:
                        ...
                        if small_count >= ACTION_SMALL_STEPS:
                            logger.info(
                                f"Detected continuous {ACTION_SMALL_STEPS} steps with very small change, ending task."
                            )
                            return
                    last_pose = pose_now
                    time.sleep(0.1)
                if max_steps is not None and step_count >= max_steps:
                    logger.info(f"Reached {max_steps} steps, switching to next task.")
                    break
            except Exception as e:
