"""
Sequential LTL Symbolic Planner: pre-constraint planner used by the full
system integration runner and the hardware runner.

Verbatim restoration of the planner that existed at April 22 (commit
2911e3e), before automaton-based constraint classification or G-operator
support landed. Treats every predicate as a goal and walks the Spot
automaton edge-by-edge; does not classify or surface constraint
predicates. Pair with ``LLMUserInterface(use_constraints=False)`` so the
LLM is prompted with the matching pre-constraint LTL system prompt.
"""
...
class SequentialLTLPlanner:
    ...
    def __init__(self, llm_interface: LLMUserInterface):
        if spot is None:
            raise ImportError(
                "The 'spot' library is required for SequentialLTLPlanner. "
