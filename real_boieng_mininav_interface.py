...
AI_SRC = REPO_ROOT / "ai_framework" / "src"
if str(AI_SRC) not in sys.path:
    sys.path.insert(0, str(AI_SRC))

from modules.utils.base_llm_providers import LLMFactory


logger = logging.getLogger(__name__)

DEFAULT_RESULTS_DIR = REPO_ROOT / "results" / "real_integration_results"
DEFAULT_ENV_VARS = REPO_ROOT / "ai_framework" / ".env_vars"
...
def load_env_vars(path: Path) -> None:
    ...
    try:
        ...
        logger.warning("Failed loading env vars from %s: %s", path, exc)
